-- Enforce one active client_program per coach/client pair and make template
-- workout import share the same active-program lock as runtime workout create.
...
begin
  if exists (
    ...
    raise exception 'Cannot add active client_program uniqueness guard while duplicate active programs exist';
  end if;
end;
$$;

create unique index if not exists idx_client_programs_one_active_per_coach_client
  on public.client_programs(client_id, coach_id)
  where status = 'active'
    and coach_id is not null;
